...
<form ref="userForm" onsubmit="{ onSubmit }">
	<input ref="username" value="{ opts.forms && opts.forms['userForm']['username'].$val }">
	...
	<select ref="sex">
		<option></option>
		<option value="0" selected="{ opts.forms && opts.forms['userForm']['sex'].$val === '0' }">男</option>
		<option value="1" selected="{ opts.forms && opts.forms['userForm']['sex'].$val === '1' }">女</option>
	</select>
